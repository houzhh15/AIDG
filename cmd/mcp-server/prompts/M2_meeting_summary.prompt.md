...
- （1）会议的核心结论和决策是什么？
- （2）支持这些结论的主要论据有哪些？
- （3）会议讨论过程中是否有不同观点表达或前后矛盾的地方，请总结讨论过程及最终决议
最后给出行动上的指南、建议。
- （1）基于会议决议内容，接下来应该采取哪些具体行动？
- （2）未形成决议的事项，下一步该如何推进？
- （3）谁负责跟进这些行动？是否有明确的截止日期？
最后补充：
...
5.  先使用 update_meeting_document 提交所有的章节包括子章节。update_meeting_document(content=所有章节标题, meeting_id, summary) 
6.  调用 get_meeting_doc_sections 获得章节信息
7.  使用 update_meeting_doc_section 更新各章节内容。但不要包含任何标题。
