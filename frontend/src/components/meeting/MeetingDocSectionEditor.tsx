/**
 * 会议文档章节编辑器
 * 与项目文档的 ProjectDocSectionEditor 保持相同的 UI 风格
 */
...
import { 
  getMeetingDocSections, 
  getMeetingDocSection, 
  updateMeetingDocSection,
  exportMeetingDoc,
  replaceMeetingDocFull,
  type MeetingDocSlot,
...
const MeetingDocSectionEditor: React.FC<Props> = ({ 
  ...
  }, [currentSectionId, meetingId, slot]);

  const loadSections = async () => {
    setLoading(true);
    try {
      const response = await getMeetingDocSections(meetingId, slot);
  ...
  const loadSectionContent = async (sectionId: string) => {
    ...
    try {
      if (sectionId === FULL_DOCUMENT_ID) {
        // 加载全文
        const response = await exportMeetingDoc(meetingId, slot);
        ...
        if (hasChildren) {
          const response = await getMeetingDocSection(meetingId, slot, sectionId, true);
          ...
          const response = await getMeetingDocSection(meetingId, slot, sectionId, false);
  ...
  const handleSave = async () => {
    ...
    try {
      if (sectionContent.id === FULL_DOCUMENT_ID || isFullEditMode) {
        // 全文模式：直接保存整个文档
        await replaceMeetingDocFull(meetingId, slot, sectionContent.content);
...
export default MeetingDocSectionEditor;
