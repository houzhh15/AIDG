...
import { EditOutlined, SaveOutlined, CloseOutlined, CopyOutlined, HistoryOutlined, DeleteOutlined, FileSearchOutlined } from '@ant-design/icons';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { getProjectArchitecture, saveProjectArchitecture, getProjectArchitectureHistory, deleteProjectArchitectureHistory, copyDeliverablesFromTask } from '../../api/projects';
...
export const ProjectArchitectureDesign: React.FC<Props> = ({ projectId }) => {
  ...
  const [selectedKinds, setSelectedKinds] = useState<string[]>(['architecture-design']);

  async function load(){
    if(!projectId) return; setLoading(true);
    try { const r = await getProjectArchitecture(projectId); setContent(r.content||''); setExists(r.exists||false); } finally { setLoading(false); }
  }
  async function loadHistoryFn(){ if(!projectId) return; setLoadingHistory(true); try { const h = await getProjectArchitectureHistory(projectId); setHistory(h); } catch{ message.error('历史加载失败'); } finally { setLoadingHistory(false); } }
  async function save(){ if(!projectId) return; setSaving(true); try { await saveProjectArchitecture(projectId, editContent); message.success('已保存'); setContent(editContent); setExists(true); setIsEditing(false); if(history.length>0) loadHistoryFn(); } catch { message.error('保存失败'); } finally { setSaving(false); } }
  async function deleteHistoryVersion(v:number){ try { await deleteProjectArchitectureHistory(projectId, v); message.success('已删除'); loadHistoryFn(); } catch { message.error('删除失败'); } }
  ...
  const handleCopy = async ()=>{ 
    ...
    if(exists && content){
      try {
        const resp = await authedApi.get(`/tasks/${sourceTaskId}/architecture-design`);
    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column', gap:12 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <span style={{ fontWeight:600, color:'#fa8c16' }}>项目架构设计</span>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          {!exists && !isEditing && !loading && (
            <>
              <Button size="small" type="primary" icon={<EditOutlined />} style={{ background:'#fa8c16', borderColor:'#fa8c16' }} onClick={()=>{ setEditContent('# 项目架构设计'); setIsEditing(true); }}>创建</Button>
              <Button size="small" icon={<CopyOutlined />} style={{ color:'#fa8c16' }} onClick={()=> setShowCopyModal(true)}>拷贝</Button>
            </>
          )}
          {exists && !isEditing && (
            <>
              <Dropdown menu={{ items: historyMenu }} trigger={['click']} onOpenChange={(o)=>{ if(o && history.length===0) loadHistoryFn(); }}>
                <Button size="small" type="text" icon={<HistoryOutlined />} style={{ color:'#fa8c16' }}>历史</Button>
              </Dropdown>
              <Button size="small" type="text" icon={<EditOutlined />} style={{ color:'#fa8c16' }} onClick={()=>{ setEditContent(content); setIsEditing(true); }}>编辑</Button>
              <Button size="small" type="text" icon={<CopyOutlined />} style={{ color:'#fa8c16' }} onClick={()=> setShowCopyModal(true)}>拷贝</Button>
            </>
          )}
          {isEditing && (
            <>
              <Button size="small" type="primary" icon={<SaveOutlined />} loading={saving} style={{ background:'#fa8c16', borderColor:'#fa8c16' }} onClick={save}>保存</Button>
              <Button size="small" icon={<CloseOutlined />} onClick={()=>{ setIsEditing(false); }}>取消</Button>
            </>
          )}
        </div>
      </div>
      <div className="scroll-region" style={{ flex:1, background:'#fff7e6', border:'1px solid #ffd591', borderRadius:8, padding:16, minHeight:0 }}>
        {loading ? (<div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:160, gap:12 }}><Spin /><span>加载中...</span></div>) : isEditing ? (
          ...
          <div style={{ color:'#999', textAlign:'center', marginTop:40 }}>暂无架构设计</div>
        ) : (
          <ReactMarkdown 
      ...
      <div style={{ fontSize:11, textAlign:'center', color:'#999' }}>🏗️ 项目级架构文档</div>
