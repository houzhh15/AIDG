...
该项目旨在解决 AI 在长程创作和复杂软件工程中容易“越界、漂移、失去全局一致性”的问题，为 AI 协作开发提供**稳定的支点与收敛机制**。

核心理念是：
**AI 负责高效探索与发散，人负责全局一致性、约束引入与长期结构稳定性。**

项目通过集中管理约束、文档与决策过程，在长时间、多阶段、多参与者的开发中，帮助团队保持共识、可追溯性与工程可维护性。

---
## 核心特点

- **长程创作的支点机制**
  为跨阶段、跨模块、跨时间的软件开发提供稳定锚点，避免 AI 生成在演进过程中不断漂移。

- **集中管理约束与决策**
  统一管理需求、设计、架构与执行约束，防止“局部合理替代全局合理”。

- **提升多人协作的决策透明度**
  所有关键决策、假设、约束与变更均以文档形式显性记录，降低协作和维护成本。

- **明确的人机分工模型**
  - AI：低成本、高并行的方案探索与执行
  - 人：约束引入、判断、冻结决策、维护全局一致性
  
---

## 价值主张

**1. 统一事实来源（Single Source of Truth）**

内置会议记录与总结机制，支持整合多源信息（讨论、决策、变更说明），形成单一、可追溯的事实来源，避免口头共识流失。

⸻

**2. 面向长程创作的任务驱动开发流程**

项目采用任务驱动的开发工作流，特别适合长周期、复杂度持续上升的软件工程：
- 使用垂直切片方式拆解任务，控制单次任务规模
- 以需求文档与设计文档作为锚点，限制 AI 的生成空间
- 避免在不可逆决策前过早放大生成规模

⸻

**3. 可扩展的人机协作机制**
- 通过 MCP 协议对接第三方 AI Copilot
- 人与 AI 共同完成锚定文档（需求 / 设计 / 约束）
- 利用 AI 的零边际成本进行发散探索
- 由人逐步引入约束，收敛为最终可执行的工程文档

⸻

**4. SDD（Spec-Driven Development）开发模式支持**

基于架构、需求与设计文档，系统化生成开发执行计划：
- 明确阶段目标与依赖关系
- AI Copilot 按步骤执行开发任务
- 人类在关键节点介入判断与冻结决策
...
这些文档共同构成完整的开发与维护轨迹，为长期演进、交接和重构提供透明、可信的基础。
